...
export function CriterionRow({ criterion, onChange, onRemove }: Props) {
  return (
    <div className="flex items-center gap-2">
      <Select value={criterion.mode} onValueChange={mode => onChange({ ...criterion, mode: mode as FilterMode })}>
...
export function App() {
  ...
  const [criteria, setCriteria] = useState<Criterion[]>([{ mode: FilterMode.Contains, value: '' }])
